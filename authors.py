...
def get_authors_data():
    ...
    df_authors = read_googlesheet(gsheet_id, "personas", ["Autor"]).rename(
        {"Autor": "Author"}, axis=1
    )
...
def display_author(df_authors, df_events, author_search_name):
    """ """
    # Start the page
    st.caption("Python Chile: Authors")
    author_clean_name = clean_name(author_search_name)
    # Try to find some talks by the author
    df_author_events = df_events.loc[
        df_events["author_clean_name"] == author_clean_name, :
    ].reset_index()
    if len(df_author_events) == 0:
        display_404_author(author_search_name)
        return
    # Get social media for the author
    df_author_links = df_authors.loc[
        df_authors["author_clean_name"] == author_clean_name, :
    ].reset_index()
    ...
    author_display_name = df_author_events["Author"].values[0]
...
def display_404_author(author):
    """ """
    st.title("404")
    st.write(f"Author {author} not found among registered authors.")
